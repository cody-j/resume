\documentclass[11pt,a4paper]{article}

\usepackage[margin=1cm]{geometry}
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage{multicol}
\usepackage{wrapfig}
\usepackage{anyfontsize}
\usepackage[dvipsnames]{xcolor}
\usepackage{tikz}
\usepackage{pgf}
\usepackage{multirow}
\usepackage{enumitem}
\usepackage{lmodern}
\usepackage{changepage}
\usepackage{ragged2e}
\usepackage{tcolorbox}
\usepackage{fontawesome}

% font setup
\usepackage[T1]{fontenc}
\usepackage{inter}

\pagenumbering{gobble}
\raggedright

\righthyphenmin=62
% Document
\begin{document}

% Commands
\newcommand{\header}[1]{%
    \vspace{-0.3em}
    {\tiny\section*{\small\normalfont\textit{#1}}\vspace{-2.5em}}
    \hrulefill\vspace{0.9em}
}

\newcommand{\contact}{
    \begin{center}
    \faGlobe\ codyj.io%
    \hspace{1em}\textbar\hspace{1em}%
    \faEnvelope\ codywj777@gmail.com%
    \hspace{1em}\textbar\hspace{1em}%
    \faPhone\ (604) 802-6801%
    \hspace{1em}\textbar\hspace{1em}%
    \faGithub\ github.com/cody-j%
    \end{center}
}

\newcommand{\skillcolumn}[2]{%
    \begin{minipage}[t]{0.29\textwidth}
        {\section*{\large\textbf{#1}}\vspace{-2.0em}}
        \hrulefill\vspace{0.3em}
        \begin{flushleft}
            \setlength{\baselineskip}{1.8em}
            {#2}
        \end{flushleft}
    \end{minipage}%
}

\newcommand{\plainpill}[1]{%
    {\small\textit{#1}}
}

\newcommand{\pill}[2][gray!90]{%
    \tcbox[
        height=0.5cm,
        nobeforeafter,
        valign=center,
        width=fit,
        colback=#1!20,
        colframe=#1!20,
        left=2pt,
        right=2pt,
        boxrule=0pt,
    ]{\footnotesize\color{#1!20!Black}\textbf{#2}}
}

\newcommand{\experience}[6]{%  % Title, Company, Date, Highlights, Blurb, Progress
    \textbf{\large #1} {\small\textbar{}} \raisebox{0ex}{\small{#2}} \hfill \raisebox{0.1ex}{\small\textbf{#3}}\\%
    {\footnotesize{#5}}\vspace{0.4em}\hfill\raisebox{0.1em}{#6}%
    \begin{adjustwidth}{0.3cm}{0.3cm}%
        \begin{itemize}[leftmargin=*, nosep]%
            {#4}
        \end{itemize}%
    \end{adjustwidth}%
    \vspace{1em}%
}

\newcommand{\progressbar}[3]{%
    \begin{tikzpicture}
        \pgfmathsetmacro{\width}{2}
        \pgfmathsetmacro{\partOne}{(#1 / 100) * \width}
        \pgfmathsetmacro{\partTwo}{\partOne + (#2 / 100) * \width}
        \pgfmathsetmacro{\partThree}{\partTwo + (#3 / 100) * \width}
        \raisebox{0.3em}{
            \fill[RoyalPurple!90!Black!60] (0,0) rectangle (\partOne,0.08);
            \fill[ForestGreen!90!Black!50] (\partOne,0) rectangle (\partTwo,0.08);
            \fill[NavyBlue!90!Black!70] (\partTwo,0) rectangle (\width,0.08);
        }
    \end{tikzpicture}%
}
% End Commands

% HEADER
\begin{center}
    {\huge \textbf{Cody Johnson}}\\[0.5em]
    \small{Vancouver, BC}
\end{center}%

\vspace{0.3em}

\vspace{-0.7em}\contact

% SKILLS
\header{SKILLS}

\small{React, Vue.js, Javascript, (ES6+), HTML5, CSS3, Node.js, TypeScript, Express.js, Nest.js, DBT, PostgreSQL, Redis, Python, Couchbase, Docker, Kubernetes, AWS, Serverless, Git, Bash, Jenkins}

\vspace{1.4em}

\header{EXPERIENCE}

\experience{Technical Founder}{Skoookum}{April 2024 - Present}{%
    \item Deployed and maintained all of the backend systems on top of AWS; Postgres + Redis on EC2, REST API on API Gateway + Lambda, multi-az private/public subnets, NAT%
    \item Built web client using React and Redux (+toolkit) that consumes the Excel Javascript API to automate workflows in the classic spreadsheet tool%
    \item Implemented OAuth on top of Microsoft Entra and API Gateway + Lamda%
}{Component library for Excel}%

\experience{Lead Software Engineer}{Treez}{December 2021 - April 2024}{%
    \item Led development of our eventing system on top of Eventbridge and Kinesis, decoupling services in what had become a distributed monolith%
    \item Led development of enterprise ELT pipeline processing data from 400+ MySQL instances reducing report generation time by 90\%, enabling real-time business insights for our users%
    \item Implemented analytics platform on DBT + Quicksight reducing cogs by hundreds of thousands of dollars per year%
}{Enterprise retail and logistics platform}%


\experience{Software Engineer}{Treez}{September 2020 - December 2021}{%
    \item Orchestrated migration from EC2 to Kubernetes enabling zero-downtime deployments%
    \item Optimized critical track-and-trace queries reducing latency in one case from +10s to <300ms%
    \item Integrated track-and-trace system with delivery partners enabling millions of dollars in new business%
}{Enterprise retail and logistics platform}%


\experience{Software Engineer}{Electronic Arts}{January 2020 - September 2020}{%
    \item Enhanced build system to better handle a particular class of error, eliminating hundreds of lost man-hours over a week%
    \item Worked with game developers and game testers to facilitate proper output from Jenkins farm%
}{The game company}%


% EXPERIENCE (cont'd)

% \newpage

% \header{EXPERIENCE (cont'd)}

\experience{Software Engineer}{Indigo Ag}{January 2019 - December 2019}{%
    \item Agricultural intelligence platform combining satellite imagery with precision farming technology%
    \item Built React-based visualization platform connecting 100TB+ of satellite imagery data with agronomists%
    \item Developed ETL pipeline processing WKT data into vector tiles, integrated into Airflow processes%
}{Agricultural intelligence platform}%

\experience{Software Engineer}{Control}{January 2017 - December 2018}{%
    \item Implemented V2 of business intelligence platform processing \$500M+ in payment data from the four major providers%
    \item Implemented auto-scaling system to handle spikes in pipeline ingestion%
}{Payment data studio}%

\experience{Bartender}{Gibbons Hospitality}{August 2012 - October 2015}{%
    \item Worked at various bar positions from the front door to bartending at Buffalo Bills and Tapley's%
    \item Developed communication skills and a sense of urgency working in fast-paced environments%
}{Major hospitality group in Whistler}%

\experience{Ski Technician}{Big White Resort}{September 2011 - May 2012}{%
    \item Mounted skis, tuned bindings to meet safety requirements%
    \item Developed skills on Wintersteiger machinery, tuning skis and snowboards for various abilities%
}{Ski-in, ski-out resort in Kelowna}%

\experience{Sales Associate / Technician}{Sportchek}{September 2009 - December 2011}{%
    \item Worked in the repair shop tuning bikes, stringing rackets, and mounting skis%
    \item Regularly worked shifts as a sales associate in the hardgoods section%
}{Sporting goods department store}%


% EDUCATION
\header{EDUCATION}

\textbf{\normalsize Web Development Bootcamp} {\small\textbar{}} \raisebox{0ex}{\small{Lighthouse Labs}} \hfill \raisebox{0.1ex}{\small\textbf{October 2016 - January 2017}}\\%
{\small{Intensive 12-week program focusing on modern web development}}
\vspace{0.8em}

\textbf{\normalsize Motorcycle Mechanics Certificate} {\small\textbar{}} \raisebox{0ex}{\small{BCIT}} \hfill \raisebox{0.1ex}{\small\textbf{January 2016 - June 2016}}\\%
{\small{Professional certification in motorcycle maintenance and repair}}%
\vspace{0.8em}

\textbf{\normalsize Mechanical Engineering Studies} {\small\textbar{}} \raisebox{0ex}{\small{BCIT}} \hfill \raisebox{0.1ex}{\small\textbf{September 2009 - February 2011}}\\%
{\small{Completed 2 years of coursework in bachelor program}}%
\vspace{0.8em}

\vspace{1.2em}


% CERTIFICATES
\header{CERTIFICATES}

\textbf{\normalsize AWS Cloud Practitioner} {\small\textbar{}} \raisebox{0ex}{\small{Amazon}} \hfill \raisebox{0.1ex}{\small\textbf{2021}}
\vspace{1.2em}

% CLUBS
\header{CLUBS}

\textbf{\normalsize First Robotics Challenge} {\small\textbar{}} \raisebox{0ex}{\small{Sullivan Heights}} \hfill \raisebox{0.1ex}{\small\textbf{2008-2009}}

\vfill

\contact

\end{document}
